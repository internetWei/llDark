...
static inline UIWindow * currentWindow(void) {
    UIWindow *window = ({
        ...
        if (@available(iOS 13.0, *)) {
            NSArray *array = ll_RCodeSync(UIApplication.sharedApplication.connectedScenes.allObjects);
            UIWindowScene *windowScene = (UIWindowScene *)array.firstObject;
            SceneDelegate *delegate = (SceneDelegate *)windowScene.delegate;
            window = delegate.window;
        } else {
            AppDelegate *delegate = ll_RCodeSync(UIApplication.sharedApplication.delegate);
            window = delegate.window;
        }
#else
        AppDelegate *delegate = ll_RCodeSync(UIApplication.sharedApplication.delegate);
